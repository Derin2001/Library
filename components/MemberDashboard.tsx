...
import { Book, Member, Transaction, Reservation, TransactionType, Settings, BookStatus } from '../types';
import Card from './common/Card';
import Modal from './common/Modal';
import { SearchIcon, BookOpenIcon, BookmarkIcon, SparklesIcon, HistoryIcon, TrashIcon, ExclamationTriangleIcon, XMarkIcon, ClockIcon } from './icons';
...
interface MemberDashboardProps {
    ...
    onCancelReservation: (reservationId: string) => Promise<any> | { success: boolean; message: string };
    onRenewLoan?: (transactionId: string) => Promise<any> | { success: boolean; message: string };
    showNotification: (message: string, type: 'success' | 'error') => void;
    settings?: Settings;
    onLogout: () => void; // ✅ Restored Logout
}
...
const MemberDashboard: React.FC<MemberDashboardProps> = ({ 
    ...
    const myActiveLoans = useMemo(() => {
        ...
        // Map book details to loan
        return active.map(loan => {
            const book = books.find(b => b.id === loan.bookId);
            return { ...loan, bookTitle: book?.title || 'Unknown Book' };
        }).sort((a,b) => parseISO(b.date).getTime() - parseISO(a.date).getTime());
    }, [transactions, currentMember.id, books]);
    ...
    // ✅ FIXED: Direct AI Logic Implementation (No External Dependency)
    const handleGetAiRecommendations = async () => {
        ...
        try {
            ...
            // 1. Prepare User History
            const userHistory = transactions
                .filter(t => t.memberId === currentMember.id)
                .map(t => {
                    const b = books.find(bk => bk.id === t.bookId);
                    return b ? `${b.title} (${b.category})` : null;
                })
                .filter(Boolean)
                .slice(-10); // Last 10 books

            const uniqueHistory = Array.from(new Set(userHistory));

            // 2. Prepare Available Books
            const availableBooks = books
                .filter(b => b.status !== BookStatus.Unavailable)
                .map(b => `"${b.title}" by ${b.author} (${b.category}, Language: ${b.language})`)
                .slice(0, 40); // Limit context size

            const prompt = `
                I am a library member.
                My reading history: [${uniqueHistory.join(', ')}].
                
                Available books in library:
                [${availableBooks.join('; ')}]

                Task: Recommend exactly 3 books from the 'Available books' list that I haven't read yet but might like based on my history.
                If history is empty, recommend 3 popular ones.
                
                Output JSON format ONLY:
                [
                    { "title": "Exact Book Title", "reason": "Short reason why", "language": "Language" }
                ]
            `;
            ...
            setAiError("Could not generate recommendations. Please try again later.");
        } finally {
    ...
    const getBookStatusInfo = (book: Book) => {
        ...
        if (onShelf > 0) {
            if (onShelf > allActiveReservations.length) {
                return { 
                    status: 'Available', 
                    detail: `Standard ${loanPeriodDays}-day loan`, 
    ...
    const handleCancelClick = async () => {
        if (resToCancel) {
            const res = await onCancelReservation(resToCancel.id);
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
            {/* ✅ Navbar Restored */}
            <nav className="bg-white dark:bg-slate-800 shadow-sm border-b dark:border-slate-700 px-6 py-4 flex justify-between items-center sticky top-0 z-10">
                <div className="flex items-center gap-2">
                    <BookOpenIcon className="h-8 w-8 text-indigo-600" />
                    <span className="text-xl font-bold text-slate-800 dark:text-white">MAHSS Library</span>
                </div>
                <div className="flex items-center gap-4">
                    <span className="text-sm text-slate-600 dark:text-slate-300 hidden sm:block">Welcome, {currentMember.name}</span>
                    <button onClick={onLogout} className="text-sm text-red-600 hover:text-red-800 font-medium px-3 py-1 rounded-md hover:bg-red-50 transition">
                        Sign Out
                    </button>
                </div>
            </nav>
                ...
                <Card className="border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/20 dark:bg-indigo-950/5">
                    ...
                    {isAiLoading ? (
                        ...
                        <div className="p-4 bg-red-50 text-red-600 rounded-lg text-sm text-center">
                            {aiError} <button onClick={handleGetAiRecommendations} className="underline font-bold ml-2">Try Again</button>
                        </div>
                    ) : aiRecs.length > 0 ? (
